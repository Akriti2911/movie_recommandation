...
async def tmdb_get(path: str, params: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    if not TMDB_API_KEY:
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"TMDB request error: {e}")
...
async def tmdb_cards_from_results(results: List[Dict[str, Any]], limit: int) -> List[TMDBMovieCard]:
    cards = []
    for res in results[:limit]:
        cards.append(
            TMDBMovieCard(
                ...
                backdrop_url=make_img_url(res.get("backdrop_path")),
                genres=[],
            )
        )
    return cards
...
def build_title_to_idx_map(indices: Any) -> Dict[str, int]:
    ...
    try:
        ...
        raise TypeError("indices must be a pandas Series or dict-like object")
    return title_to_idx
